...
const { validate } = require("../validators/practical-attendance");
...
const table = "practical_attendance";
const id = `id`;
const resourceName = `Practical Attendance`;

router.get('/', [auth, permission], async (req, res) => {
    const practicalAttendances = await database.find(table);
    res.send(practicalAttendances);
});
...
router.post('/', [validateBody(validate), auth, permission], async (req, res) => {
    ...
    const primaryKey = ['practical_attendance_conducted', 'roll_number', 'date'];
...
router.put(`/:${id}/multiple`, [auth, permission], async (req, res) => {
    ...
    const query = `UPDATE practical_attendance SET status = 1 WHERE practical_session_conducted = ${req.params.id} AND roll_number IN (${quotedAndCommaSeparated})`;
...
router.put(`/`, [auth, permission], async (req, res) => {
    const { practical_session_conducted, status, date, roll_number } = req.body;
    await handler.put(req, res, table, { practical_session_conducted, date, roll_number }, { status }, resourceName);
});
