...
const { validate } = require("../validators/theory-assign");
...
const table = "theory_assign";
const id = `id`;
const resourceName = "Theory assign";


router.get('/', [auth, permission], async (req, res) => {
    const theoryAssigns = await database.find(table);
    res.send(theoryAssigns);
});
...
router.post('/', [validateBody(validate), auth, permission], async (req, res) => {
    const primaryKey = ['course_code', 'fac_id', 'dept_id', 'acad_year', 'sem', 'divs'];
