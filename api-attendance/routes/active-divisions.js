...
const { validate } = require("../validators/active-division");
...
const table = "active_division";
const id = `id`;
const resourceName = "Active Division"

router.get('/', [auth, permission], async (req, res) => {
    const activeDivisions = await database.find(table);
    res.send(activeDivisions);
});
...
router.post('/', [validateBody(validate), auth, permission], async (req, res) => {
    const primaryKey = ['dept_id', 'acad_year', 'sem', 'divs', 'roll_number'];
